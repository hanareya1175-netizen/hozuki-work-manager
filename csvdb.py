...
def register_result_and_complete(
    *, recruit_id: str, result_row: dict[str, Any], completed_status: str
) -> str:
    """実績追加と募集完了を一つの接続・トランザクションで確定する。"""
    initialize()
    clean = {str(k): "" if v is None else str(v) for k, v in result_row.items()}

    if using_postgres():
        with _connect() as conn, conn.cursor() as cur:
            cur.execute("SELECT pg_advisory_xact_lock(20260728)")

            cur.execute(
                "SELECT data FROM hozuki_records "
                "WHERE collection='recruit' AND record_key=%s FOR UPDATE",
                (f"id:{recruit_id}",),
            )
            fetched = cur.fetchone()
            if fetched is None:
                raise ValueError(f"募集 {recruit_id} が見つかりません。")
            recruit = {str(k): "" if v is None else str(v) for k, v in fetched[0].items()}

            cur.execute(
                "SELECT COALESCE(MAX((data->>'result_id')::integer), 0) + 1 "
                "FROM hozuki_records WHERE collection='results' "
                "AND (data->>'result_id') ~ '^[0-9]+$'"
            )
            result_id = str(int(cur.fetchone()[0]))
            clean["result_id"] = result_id

            cur.execute(
                "SELECT COALESCE(MAX(position), 0) + 1 FROM hozuki_records "
                "WHERE collection='results'"
            )
            position = int(cur.fetchone()[0])
            cur.execute(
                """
                INSERT INTO hozuki_records(collection, record_key, position, data)
                VALUES('results', %s, %s, %s::jsonb)
                """,
                (_record_key("results", clean, position), position, json.dumps(clean, ensure_ascii=False)),
            )

            recruit["status"] = completed_status
            cur.execute(
                "UPDATE hozuki_records SET data=%s::jsonb, updated_at=NOW() "
                "WHERE collection='recruit' AND record_key=%s",
                (json.dumps(recruit, ensure_ascii=False), f"id:{recruit_id}"),
            )
            conn.commit()
        _clear_read_cache()
        return result_id

    clean["result_id"] = next_id("results", "result_id")
    results = read("results")
    results.append(clean)
    recruits = read("recruit")
    found = False
    for row in recruits:
        if str(row.get("id", "")) == str(recruit_id):
            row["status"] = completed_status
            found = True
            break
    if not found:
        raise ValueError(f"募集 {recruit_id} が見つかりません。")
    write_all("results", results)
    write_all("recruit", recruits)
    return clean["result_id"]
